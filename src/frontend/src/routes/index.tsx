      ...
      <div className="relative min-h-[650px] overflow-hidden rounded-[2rem] md:min-h-[700px]">
        ...
        <div className="relative flex min-h-[650px] flex-col justify-between p-6 text-cream sm:p-10 md:min-h-[700px] md:p-14">
          ...
          <div className="max-w-3xl">
            <p className="mb-4 text-xs uppercase tracking-[0.2em] text-cream/70 font-mono">See the failure before it happens</p>
            ...
            <p className="mt-6 max-w-xl text-sm leading-relaxed text-cream/80 sm:text-base">
              VOLTRA combines two machine learning models trained on Kaggle transformer datasets — Health Index regression (R²=0.72) and DGA Fault Classification (90.8% accuracy) — with IBM Bob plain-English advisories to turn catastrophic failures into scheduled interventions.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Button asChild size="lg" className="pill h-12 bg-signal px-6 text-signal-foreground hover:bg-signal/90 font-semibold shadow-glow">
                <Link to="/grid">Explore Live Grid Console <ArrowRight className="ml-1 size-4" /></Link>
              </Button>
              <Button asChild size="lg" variant="outline" className="pill h-12 border-cream/20 bg-cream/10 px-6 text-cream backdrop-blur hover:bg-cream/20 hover:text-cream">
                <Link to="/predict">Run Dual ML Prediction <Activity className="ml-1 size-4" /></Link>
              </Button>
            </div>
          </div>
    ...
    <section className="mx-auto mt-24 w-full max-w-6xl px-6">
      <div className="grid gap-10 md:grid-cols-12">
        <div className="md:col-span-4">
          <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground font-mono">The problem</p>
          <h2 className="mt-4 text-4xl font-semibold leading-tight md:text-5xl">
            Yesterday's grid tools respond <span className="font-display font-normal italic">after the dark.</span>
          </h2>
          <p className="mt-4 text-xs text-muted-foreground leading-relaxed">
            Calendar-based maintenance dispatches crews reactively at 3-7x the cost of planned intervention, while transformer sensors show warning signatures weeks before catastrophic breakdown.
          </p>
        </div>
        <div className="grid gap-4 md:col-span-8 md:grid-cols-2">
          <FlowPanel
            ...
            tone="muted"
            items={["Winding arcing or overheating initiates", "Insulation breaks down; blackout occurs", "Outage registered by customer calls", "Emergency crew dispatched at $1M+/hr"]}
            footer="The outage has already happened."
          />
          <FlowPanel
            title="The VOLTRA approach"
            tone="signal"
            items={["Continuous DGA telemetry monitoring", "Model 1 predicts Health Index damage", "Model 2 classifies exact fault mode (D1/T1)", "IBM Bob advises targeted intervention"]}
...
function Pipeline() {
  const steps = [
    { i: Database, t: "Real DGA Data", d: "Kaggle failure analysis (4,151 rows) & Health Index reference." },
    { i: Radio, t: "Telemetry Stream", d: "18 transformers across 4 Anand District sub-zones." },
    { i: Activity, t: "Model 1 Regression", d: "Health Index damage score (R²=0.72) + calibrated RUL." },
    { i: BrainCircuit, t: "Model 2 Classifier", d: "7 IEC fault types with Duval triangle gas ratios (90.8% acc)." },
    { i: Gauge, t: "Composite Ranking", d: "Defensible 5-factor impact formula + IBM Bob advisories." },
  ];
  return (
    <section className="mt-24 border-y border-border/60 bg-ink py-20 text-cream">
      <div className="mx-auto max-w-6xl px-6">
        <div className="max-w-2xl">
          <p className="text-xs uppercase tracking-[0.18em] text-signal font-mono">The intelligence layer</p>
          <h2 className="mt-4 text-4xl font-semibold md:text-5xl">
            From raw gas ppm to an <span className="font-display font-normal italic">actionable forecast.</span>
          </h2>
        </div>
  ...
  return (
    <section className="mx-auto mt-24 w-full max-w-6xl px-4 sm:px-6">
      <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
          <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground font-mono">A fault, caught upstream</p>
          <h2 className="mt-3 text-4xl font-semibold md:text-5xl">
            Power is still flowing. <span className="font-display font-normal italic">VOLTRA is already watching.</span>
          </h2>
        </div>
  ...
  return (
    <section className="mx-auto mt-24 w-full max-w-6xl px-6">
      <div className="grid gap-8 lg:grid-cols-[.85fr_1.6fr]">
        <div>
          <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground font-mono">Live Grid Topology</p>
          <h2 className="mt-4 text-4xl font-semibold">
            A clear view of <span className="font-display font-normal italic">what changes next.</span>
          </h2>
...
function Metric({ icon: Icon, value, label, status }: { icon: typeof Activity; value: string; label: string; status: string }) {
  return (
      ...
      <div className="glass-dark overflow-hidden rounded-[2rem] p-6 md:p-10">
        <div className="grid gap-10 md:grid-cols-2">
          <div>
            ...
            <Button asChild className="pill mt-8 bg-signal text-signal-foreground hover:bg-signal/90 font-semibold shadow-glow">
              <Link to="/predict">Simulate in Outage Studio <ArrowRight className="ml-1 size-4" /></Link>
            </Button>
          </div>
  ...
  return (
    <section className="mx-auto mt-24 w-full max-w-6xl px-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground font-mono">Fleet Trajectory</p>
          <h2 className="mt-4 text-4xl font-semibold">
            Track degradation & <span className="font-display font-normal italic">maintenance recovery.</span>
          </h2>
        </div>
        <p className="max-w-sm text-sm text-muted-foreground">
          ...
          <div className="mt-6 h-72">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={history}>
                ...
                <Tooltip />
                <Area type="monotone" dataKey="health" name="Health %" stroke="var(--color-signal)" fill="url(#load)" strokeWidth={2} />
                <Area type="monotone" dataKey="load" name="Load (MVA)" stroke="var(--color-foreground)" fill="transparent" strokeWidth={1.5} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
    ...
    <section className="mx-auto mt-24 w-full max-w-6xl px-6 pb-16">
      <div className="rounded-[2rem] border border-border/60 bg-signal p-8 text-signal-foreground md:p-14">
        <div className="grid gap-8 md:grid-cols-[1.4fr_1fr] md:items-end">
          <div>
            <p className="text-xs uppercase tracking-[0.18em] opacity-70 font-mono">Grid Risk Advisor · Team Techtonics</p>
          ...
          <div>
            ...
            <Button asChild className="pill mt-6 bg-ink text-cream hover:bg-ink/90 font-semibold shadow-soft">
              <Link to="/grid">
