...
export const presetScenarios: ScenarioPreset[] = [
  {
    ...
    description: "Explosive C2H2 (>2500 ppm) and H2 surge with dielectric breakdown (57→28 kV). High-energy arcing (D1/D2) indicates imminent winding failure.",
    inputs: {
      assetId: "TX-107",
  ...
  },
  {
    ...
    description: "Gradual 3-week escalation in CH4 and C2H4 correlated with 40°C heatwave ambient. Top-oil temp at 95°C limit, indicating T1/T2 thermal decomposition.",
    inputs: {
      assetId: "TX-104",
  ...
  },
  {
    ...
    description: "Crucial demonstration case: Day 78 peak (HI 71.3, RUL 7.7d) arrested by cooling fan motor replacement and 20% load reduction. Rescued RUL to 97 days.",
    inputs: {
      assetId: "TX-115",
  ...
  },
  {
    ...
    description: "Excavation strike mechanical shock triggered internal vibration and acoustic disturbance, resulting in low-energy partial discharge (PD/D1).",
    inputs: {
      assetId: "TX-112",
...
export function calculatePrediction(inputs: ScenarioInput): PredictionResult {
  ...
  const trajectory: TrajectoryPoint[] = [
    { hour: 0, label: "T+0h", projectedRisk: Math.round(outageRiskPercent * 0.75), criticalThreshold: 80 },
    { hour: 6, label: "T+6h", projectedRisk: Math.round(outageRiskPercent * 0.82), criticalThreshold: 80 },
    { hour: 12, label: "T+12h", projectedRisk: Math.round(outageRiskPercent * 0.91), criticalThreshold: 80 },
    { hour: 24, label: "T+24h", projectedRisk: outageRiskPercent, criticalThreshold: 80 },
  ];

  // Contributing factors
  const riskFactors: RiskFactor[] = [
    { name: c2h2 > 50 ? "Acetylene Arcing Gas (C2H2)" : "Methane Thermal Gas (CH4)", weightPercent: 38 },
  ...
  if (hi >= 50) {
    actions.push({ action: "Immediate Emergency Crew Dispatch (<24h window)", impactReductionPercent: 42, priority: "high" });
    actions.push({ action: "Execute 20% Load Shed to adjacent substation node", impactReductionPercent: 25, priority: "high" });
    actions.push({ action: "Stage replacement transformer mobile trailer", impactReductionPercent: 18, priority: "medium" });
  } else if (hi >= 30) {
    actions.push({ action: "Schedule diagnostic oil lab DGA sample within 48h", impactReductionPercent: 28, priority: "high" });
    actions.push({ action: "Activate forced auxiliary radiator cooling fans", impactReductionPercent: 22, priority: "medium" });
  } else {
    actions.push({ action: "Maintain standard telemetry cycle & quarterly DGA", impactReductionPercent: 10, priority: "low" });
  }
  ...
  return {
    ...
    affectedCustomersEst: inputs.assetId === "TX-107" ? 31000 : inputs.assetId === "TX-104" ? 24500 : 18000,
    riskFactors,
    trajectory,
